...
export default function CreateBrandPage() {
  ...
  const validate = () => {
    const e = {};
    if (!form.name.trim()) e.name = "Marka adı zorunludur";
    return e;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(err.detail || "Marka oluşturulamadı");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <nav className="mb-2 flex items-center gap-2 text-xs text-ink-soft">
            ...
            <span>›</span>
            <span className="text-ink font-medium">Yeni Marka</span>
          </nav>
          <h1 className="text-2xl font-bold tracking-tight text-ink">Yeni Marka Ekle</h1>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <textarea
              ...
              onChange={handleChange}
              placeholder="Marka hakkında kısa açıklama (opsiyonel)"
