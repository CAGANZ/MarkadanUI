"use client";
// src/app/admin/categories/[id]/edit/page.jsx
...
export default function EditCategoryPage({ params }) {
  const { id } = use(params); // Next 15: params await edilir
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          setInitial(data);
          setForm({ name: data?.name ?? "", description: data?.description ?? "", imageUrl: data?.imageUrl ?? "" });
        }
      } catch (e) {
        if (!cancelled) setError(e.detail || e.message || "Kategori yüklenemedi");
      } finally {
  ...
  if (error) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6">
        ...
        <Link href="/admin/categories" className="mt-4 inline-block text-sm font-semibold text-primary hover:underline">
          ‹ Kategorilere dön
        </Link>
      </div>
    );
    ...
    <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-ink">Kategori Düzenle</h1>
